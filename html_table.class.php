...
class HTML_Table {
    ...
    private $cols_ar = array(); // ЕСЛИ колонки не объеденены
    
    private $tableStr = '';
    
    function __construct($id = '', $klass = '', $attr_ar = array() ) {
        // добавл строк в tbody перед вызовом addTSection
...
*/
    ...
    function addCell($data = '', $klass = '', $type = 'data', $attr_ar = array() ) {
        ...
        if ( empty($this->cur_section['rows']) ) {
            try {
                throw new Exception('Необходим addRow перед вызовом addCell');
            } catch(Exception $ex) {
                $msg = $ex->getMessage();
                echo "<p>Error: $msg</p>";
            }
